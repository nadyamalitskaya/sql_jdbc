...
public class UserLoginDAOAdd extends DBTestCase {
    public UserLoginDAOAdd(String testName) throws Exception {
        super(testName);
    ...
    protected IDataSet getDataSet() throws Exception {
        return new FlatXmlDataSetBuilder().build(new FileInputStream(
                "./src/test/java/testData/UserLoginAdd.xml"));
    }
    ...
    public void testAddUserLogin() throws Exception {
        DAOInterface<UserLogin> userLoginDAOInterface = DAOFactory.getInstance()
                .getUserLoginDAO();
        UserLogin userLogin = new UserLogin("nadechka", "vjbsjd63827ies",
                "nadyamal@gmai.com");

        userLoginDAOInterface.add(userLogin);

        IDataSet databaseDataSet = getConnection().createDataSet();
        ITable actualTable = databaseDataSet.getTable("user_login");
        IDataSet expectedDataSet = new FlatXmlDataSet(new File(
                "./src/test/java/expectedTestData/UserLoginAddExpected.xml"));
