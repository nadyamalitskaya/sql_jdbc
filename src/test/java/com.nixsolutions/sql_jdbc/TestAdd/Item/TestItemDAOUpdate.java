...
public class TestItemDAOUpdate extends DBTestCase {
    public TestItemDAOUpdate(String testName) throws Exception {
        super(testName);
    ...
    public void testUpdateItem() throws Exception {
        DAOInterface<Item> itemDAOInterface = DAOFactory.getInstance()
                .getItemDAO();
        Item item = itemDAOInterface.getById(2);
        item.setPublishOffice("Aqua");

        itemDAOInterface.update(item);

        IDataSet databaseDataSet = getConnection().createDataSet();
        ITable actualTable = databaseDataSet.getTable("item");
        IDataSet expectedDataSet = new FlatXmlDataSet(new File(
                "./src/test/java/expectedTestData/ItemUpdateExpected.xml"));
