...
public class TestItemDAOAdd extends DBTestCase {
    public TestItemDAOAdd(String testName) throws Exception {
        super(testName);
    ...
    protected IDataSet getDataSet() throws Exception {
        return new FlatXmlDataSetBuilder().build(new FileInputStream(
                "./src/test/java/testData/ItemAdd.xml"));
    ...
    public void testAddItem() throws Exception {
        DAOInterface<Item> itemDAOInterface = DAOFactory.getInstance()
                .getItemDAO();
        Item item = new Item(1998, "Lebra", (long) 1, (long) 1);

        itemDAOInterface.add(item);

        IDataSet databaseDataSet = getConnection().createDataSet();
        ITable actualTable = databaseDataSet.getTable("item");
        IDataSet expectedDataSet = new FlatXmlDataSet(new File(
                "./src/test/java/expectedTestData/ItemAddExpected.xml"));
