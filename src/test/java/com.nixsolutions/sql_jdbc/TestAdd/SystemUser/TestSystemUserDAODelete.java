...
public class TestSystemUserDAODelete extends DBTestCase {
    public TestSystemUserDAODelete(String testName) throws Exception {
        super(testName);
    ...
    protected IDataSet getDataSet() throws Exception {
        return new FlatXmlDataSet(new File(
                "./src/test/java/testData/SystemUserUpdateDeleteDataSet.xml"));
    }

    public void testDeleteSystemUser() throws Exception {
        DAOInterface<SystemUser> systemUserDAOInterface = DAOFactory
                .getInstance().getSystemUserDAO();

        systemUserDAOInterface.delete(2);

        IDataSet databaseDataSet = getConnection().createDataSet();
        ITable actualTable = databaseDataSet.getTable("system_user");
        IDataSet expectedDataSet = new FlatXmlDataSet(new File(
                "./src/test/java/expectedTestData/SystemUserDeleteExpextedDataSet.xml"));
