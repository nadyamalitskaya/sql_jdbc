...
public class BookAuthorUpdate extends DBTestCase {
    public BookAuthorUpdate(String testName) throws Exception {
        super(testName);
    ...
    public void testUpdateBookAuthor() throws Exception {
        DAOInterface<BookAuthor> bookAuthorDAO = DAOFactory.getInstance()
                .getBookAuthorDAO();
        BookAuthor bookAuthor1 = bookAuthorDAO.getById(2);
        bookAuthor1.setAuthorId((long) 1);
        bookAuthorDAO.update(bookAuthor1);

        IDataSet databaseDataSet = getConnection().createDataSet();
        ITable actualTable = databaseDataSet.getTable("book_author");
        IDataSet expectedDataSet = new FlatXmlDataSet(new File(
                "./src/test/java/expectedTestData/BookAuthorUpdateExpected.xml"));
